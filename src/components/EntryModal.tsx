...
// 🌟 新增接收 entries 參數，用來記憶歷史標籤
export default function EntryModal({ 
  isOpen, onClose, editData = null, currentMainTab = '種草', currentSubTab = 'eat', entries = [] 
}: { 
  isOpen: boolean; onClose: () => void; editData?: any; currentMainTab?: string; currentSubTab?: string; entries?: any[]; 
}) {
  ...
  let defaultOptions = [{ value: '咖啡廳/甜點', label: '咖啡廳/甜點' }, { value: '韓式烤肉/烤腸', label: '韓式烤肉/烤腸' }];
  
  if (currentSubTab === 'eat') { nameLabel = '餐廳 / 店名'; categoryLabel = '菜式 / 料理種類'; }
  else if (currentSubTab === 'go') { nameLabel = '景點 / 地標名稱'; categoryLabel = '景點類型'; defaultOptions = [{ value: '博物館/美術館', label: '博物館/美術館' }, { value: '自然風景', label: '自然風景' }]; }
  else if (currentSubTab === 'buy') { nameLabel = '品牌 / 商店名稱'; categoryLabel = '商品種類'; defaultOptions = [{ value: '服飾/穿搭', label: '服飾/穿搭' }, { value: '美妝/保養', label: '美妝/保養' }]; }
  else if (currentMainTab === '行事曆') { nameLabel = '待辦事項 / 行程名稱'; categoryLabel = '事項類別'; defaultOptions = [{ value: '航班/交通', label: '航班/交通' }, { value: '飯店住宿', label: '飯店住宿' }]; }

  // 🌟 從歷史紀錄中萃取「自訂地區」與「自訂菜式」
  const existingDishTypes = Array.from(new Set(entries.map(e => e.tags?.dishType).filter(Boolean)));
  const combinedDishOptions = Array.from(new Set([...defaultOptions.map(o => o.label), ...existingDishTypes])).map(l => ({ value: l, label: l as string }));

  const defaultRegionsForCountry = selectedCountry ? (LOCATION_DATA[selectedCountry.value] || []) : [];
  const existingRegionsForCountry = Array.from(new Set(entries.filter(e => e.tags?.country === selectedCountry?.value && e.tags?.region).map(e => e.tags.region)));
  const combinedRegionOptions = Array.from(new Set([...defaultRegionsForCountry, ...existingRegionsForCountry])).map(r => ({ value: r, label: r as string }));

  // 🔧 修復核心：每次打開表單，強力清空所有暫存狀態，包含圖片
  useEffect(() => {
    if (isOpen) {
      setImageFile(null); // 👈 絕對清空實體檔案暫存，防止圖片被竄改
      ...
      if (editData) {
        ...
        setSelectedCountry(null); setSelectedRegion(null); setDishType(null); setLinks(['']);
      }
    }
  }, [editData, isOpen]);
  ...
  const handleAISearch = async () => {
    ...
    try {
      const res = await fetch('/api/grok', { 
        method: 'POST', 
        body: JSON.stringify({ query_text: storeName, urls: validLinks, country: selectedCountry?.value || '', region: selectedRegion?.value || '' }), 
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      // 🔧 修復核心：如果沒有選擇新檔案，就繼續使用舊有的圖片
      let finalImageUrls = editData && editData.images ? [...editData.images] : [];
      ...
      const entryData = {
        ...
        tags: { 
          ...
          region: selectedRegion?.value || '', // 現在會儲存你自訂的地區
          ...
          <motion.div initial={{ y: '100%', opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: '100%', opacity: 0 }} transition={{ type: 'spring', damping: 25, stiffness: 200 }} onPaste={handlePaste} tabIndex={-1} className="bg-white w-full max-w-md rounded-t-3xl sm:rounded-3xl p-6 h-[85vh] sm:h-[80vh] overflow-y-auto shadow-soft flex flex-col outline-none">
            ...
            <div className="space-y-4 flex-1">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  {/* 🌟 升級為 CreatableSelect，支援手動輸入新地區 */}
                  <label className="block text-xs font-medium text-gray-500 mb-1">地區 (可自行輸入)</label>
                  <CreatableSelect 
                    isClearable 
                    options={combinedRegionOptions} 
                    styles={customStyles} 
                    value={selectedRegion} 
                    onChange={setSelectedRegion} 
                    placeholder="選擇或輸入" 
                    isDisabled={!selectedCountry}
                    formatCreateLabel={(v) => `新增地區 "${v}"`} 
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <CreatableSelect isClearable options={combinedDishOptions} styles={customStyles} value={dishType} onChange={setDishType} placeholder="搜尋或輸入..." formatCreateLabel={(v) => `新增 "${v}"`} />
              </div>
              ...
              <div>
                <div className="flex justify-between items-center mb-1">
                  <label className="block text-xs font-medium text-gray-500">參考連結 / 文案 <span className="text-pink-400 font-bold">(Optional)</span></label>
