...
export default function EntryModal({ isOpen, onClose, editData = null }: { isOpen: boolean; onClose: () => void; editData?: any }) {
  ...
  const [businessHours, setBusinessHours] = useState(''); // 👈 新增營業時間
  ...
  useEffect(() => {
    if (editData && isOpen) {
      ...
      setBusinessHours(editData.businessHours || ''); // 👈 讀取舊資料
      ...
      setStoreName(''); setAddress(''); setBusinessHours(''); setSelectedCountry(null); setSelectedRegion(null); setDishType(null); setLinks(['']); setPreviewImage(null); setImageFile(null);
    }
  }, [editData, isOpen]);
  ...
  const handleAIDetect = async () => {
    ...
    try {
      ...
      if (data.businessHours) setBusinessHours(data.businessHours); // 👈 填入 AI 營業時間
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const entryData = {
        title: storeName,
        address: address,
        businessHours: businessHours, // 👈 存入資料庫
        images: uploadedImageUrl ? [uploadedImageUrl] : [], 
        tags: { country: selectedCountry?.value || '', region: selectedRegion?.value || '', dishType: dishType?.value || '' },
        ...
        <motion.div initial={{ y: '100%', opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: '100%', opacity: 0 }} transition={{ type: 'spring', damping: 25, stiffness: 200 }} onPaste={handlePaste} tabIndex={-1} className="bg-white w-full max-w-md rounded-t-3xl sm:rounded-3xl p-6 h-[85vh] sm:h-[80vh] overflow-y-auto shadow-soft flex flex-col outline-none">
          ...
          <div className="space-y-4 flex-1">
            <div onClick={() => fileInputRef.current?.click()} className="border-2 border-dashed border-[#D1D9E6] rounded-2xl h-40 flex flex-col items-center justify-center text-gray-400 bg-gray-50 cursor-pointer hover:bg-gray-100 transition-colors relative overflow-hidden shrink-0">
              {previewImage && !previewImage.startsWith('blob:') && !imageFile ? (
            ...
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1">店名 / 餐廳名</label>
            ...
            <div>
              <div className="flex justify-between items-center mb-1">
                ...
                <button onClick={handleAddLink} className="text-[#D1D9E6] hover:text-[#F3E0E2] flex items-center text-xs transition-colors"><Plus size={14} className="mr-1" /> 新增</button>
              </div>
              <div className="space-y-2">
                {links.map((link, index) => (
                  <div key={index} className="flex items-center space-x-2">
                    ...
                    {index === 0 && (
                      <button onClick={handleAIDetect} disabled={isDetecting} className={`px-3 py-2.5 rounded-xl text-xs font-bold transition-all shrink-0 ${isDetecting ? 'bg-gray-100 text-gray-400' : 'bg-[#F3E0E2] text-[#585C64] hover:bg-pink-300 hover:text-white shadow-sm'}`}>
